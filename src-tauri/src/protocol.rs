...
fn resolve_and_read(uri_path: &str) -> AppResult<(Vec<u8>, &'static str)> {
    ...
    // Never expose Versailles private state (contains API bearer token).
    let first = decoded.split(['/', '\\']).next().unwrap_or("");
    if first.eq_ignore_ascii_case(".versailles") {
    ...
    // Defense in depth: block anything that resolves into .versailles.
    if canonical
        .strip_prefix(&root_canon)
        .ok()
        .and_then(|rel| rel.components().next())
        .map(|c| c.as_os_str().to_string_lossy().eq_ignore_ascii_case(".versailles"))
        .unwrap_or(false)
    {
        return Err(AppError::msg("Forbidden path"));
    }
...
/// Reliable widget URL for WebView2: plain localhost HTTP from Versailles's API.
