...
function widgetIdFromLabel(): string | null {
  try {
    const w = window as unknown as {
      ...
      __DECK_WIDGET_ID__?: string;
    };
    if (typeof w.__VERSAILLES_WIDGET_ID__ === "string" && w.__VERSAILLES_WIDGET_ID__) {
      return w.__VERSAILLES_WIDGET_ID__;
    }
    if (typeof w.__DECK_WIDGET_ID__ === "string" && w.__DECK_WIDGET_ID__) {
      return w.__DECK_WIDGET_ID__;
    }
    const params = new URLSearchParams(window.location.search);
    const fromQuery = params.get("versaillesWidgetId") || params.get("deckWidgetId");
...
function callerArgs(id?: string | null): Record<string, unknown> {
  const caller = id || widgetIdFromLabel() || undefined;
...
function makeHooks(id?: string) {
  return {
    media: {
      async now() {
        return invoke<MediaInfo>("media_now", callerArgs(id));
      },
      async nowPlaying() {
        return invoke<MediaInfo>("media_now", callerArgs(id));
      },
      async playPause() {
        return invoke("media_play_pause_cmd", callerArgs(id));
      },
      async next() {
        return invoke("media_next_cmd", callerArgs(id));
      },
      async previous() {
        return invoke("media_previous_cmd", callerArgs(id));
      },
    ...
    mouse: {
      async position() {
        return invoke<[number, number]>("get_mouse_position_cmd", callerArgs(id));
      },
    },
    async spawn(spawnId: string) {
      return invoke("toggle_slideout", { id: spawnId, ...callerArgs(id) });
    },
    shell: {
      async open(target: string) {
        return invoke("cli_open", { target, ...callerArgs(id) });
      },
    },
  };
}
...
export const versailles = {
  ...
  async moveTo(id: string, x: number, y: number, disableSnap = false): Promise<SnapResult> {
    return invoke("move_widget_cmd", { id, x, y, disableSnap, ...callerArgs(id) });
  },
  ...
  enableNativeChrome(el: HTMLElement) {
    ...
    const w = window as Window & {
      __VERSAILLES_NATIVE_CHROME__?: boolean;
      __DECK_NATIVE_CHROME__?: boolean;
    };
    w.__VERSAILLES_NATIVE_CHROME__ = true;
    w.__DECK_NATIVE_CHROME__ = true;
...
export type Versailles = typeof versailles;

/** @deprecated Use `versailles` */
export const deck = versailles;
/** @deprecated Use `Versailles` */
export type Deck = Versailles;
