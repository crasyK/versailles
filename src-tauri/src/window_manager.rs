...
fn ensure_widget_menu_handler(app: &AppHandle, window: &WebviewWindow, id: &str) {
    ...
    window.on_menu_event(move |_window, event| {
        ...
        if eid == "wmanager" {
            let _ = ensure_manager_window(&app_handle);
            return;
        }
...
fn widget_host_chrome_script(id: &str, opacity: f64, radius: u32) -> String {
  ...
  function deckApplyHostChrome() {{
    try {{
      document.documentElement.style.opacity = '{opacity}';
      if (!document.getElementById('deck-host-chrome')) {{
        var s = document.createElement('style');
        s.id = 'deck-host-chrome';
      ...
      if (window.__DECK_NATIVE_CHROME__) return;
      if (window.__DECK_HOST_MENU_BOUND__) return;
      window.__DECK_HOST_MENU_BOUND__ = true;
      document.addEventListener('contextmenu', function(e) {{
        if (window.__DECK_NATIVE_CHROME__) return;
        e.preventDefault();
        var wid = window.__DECK_WIDGET_ID__;
        if (!wid) return;
  ...
  if (document.readyState === 'loading') {{
    document.addEventListener('DOMContentLoaded', deckApplyHostChrome, {{ once: true }});
  }} else {{
    deckApplyHostChrome();
  }}
}})();"#
...
pub fn open_widget_window(
    ...
    // Widgets iframed in desktop/index.html are not OS windows.
    // Slide-outs pass always_on_top: Some(true) and stay overlays.
    if always_on_top != Some(true) {
        let state = app.state::<AppState>();
        if crate::desktop::widget_is_on_desktop_page(&state, id) {
    ...
    // Resolve widget with a short lock — never hold registry across window APIs.
    let widget = {
        let state = app.state::<AppState>();
        ...
        registry
            .get(id)
            .cloned()
            .ok_or_else(|| AppError::msg(format!("Widget '{id}' not found")))?
    };

    let label = widget_label(id);
    if let Some(existing) = app.get_webview_window(&label) {
        let width = widget.manifest.width;
        let height = widget.manifest.height;
        ...
        let raw_pos = position
            ...
            .or(widget.manifest.default_position.clone())
        ...
        let on_top = always_on_top.unwrap_or(widget.manifest.always_on_top);
        let state = {
    ...
    let raw_pos = position
        .or(widget.manifest.default_position.clone())
        .unwrap_or(Position { x: 40, y: 40 });
    let pos = clamp_position_to_monitors(
        app,
        raw_pos.x,
        raw_pos.y,
        widget.manifest.width,
        widget.manifest.height,
    );
    let on_top = always_on_top.unwrap_or(widget.manifest.always_on_top);
    let port = app.state::<AppState>().config.lock().unwrap().api_port;
    ...
    let opacity = widget.manifest.opacity.clamp(0.05, 1.0);
    let radius = widget.manifest.border_radius;
    let inject = widget_host_chrome_script(id, opacity, radius);
    ...
    let window = match WebviewWindowBuilder::new(app, &label, WebviewUrl::External(url))
        .title("")
        .inner_size(widget.manifest.width as f64, widget.manifest.height as f64)
        .decorations(false)
        .transparent(true)
        .always_on_top(on_top)
        .skip_taskbar(true)
...
pub fn popup_widget_context_menu(app: &AppHandle, id: &str) -> AppResult<()> {
    ...
    let manager_item =
        MenuItem::with_id(app, "wmanager", "Open Deck manager", true, None::<&str>)?;
    let menu = Menu::with_items(app, &[&close, &aot, &manager_item])?;
...
fn restore_manager_geometry(window: &WebviewWindow) {
    // Hidden/minimized Win32 windows often sit near (-32000,-32000) with a
    // collapsed chrome size; show()+focus alone cannot bring them back.
    let pos = window.outer_position().ok();
    let size = window.outer_size().ok();
    let offscreen = pos
        .map(|p| p.x < -500 || p.y < -500)
        .unwrap_or(true);
    let too_small = size
    ...
    #[cfg(windows)]
    {
        // Belt-and-suspenders: Win32 SetWindowPos when Tauri setters no-op on
        // a still-minimized / cloaked HWND.
...
pub fn ensure_manager_window(app: &AppHandle) -> AppResult<()> {
    if let Some(window) = app.get_webview_window("main") {
        let _ = window.set_skip_taskbar(true);
        // Unminimize/show first — off-screen minimized HWNDs ignore size/pos until restored.
        let _ = window.unminimize();
        let _ = window.show();
        restore_manager_geometry(&window);
...
pub const ANYWHERE_LABEL: &str = "anywhere";
pub const ANYWHERE_HEIGHT_LOGICAL: f64 = 34.0;
...
pub fn place_anywhere_bar(window: &WebviewWindow) {
    let (left, top, width, _height, scale) = launcher_monitor_bounds(window);
    let h = (ANYWHERE_HEIGHT_LOGICAL * scale).round() as u32;
    let _ = window.set_position(tauri::Position::Physical(PhysicalPosition { x: left, y: top }));
    let _ = window.set_size(tauri::Size::Physical(PhysicalSize {
        width,
        height: h.max(28),
...
pub fn ensure_anywhere_window(app: &AppHandle) -> AppResult<()> {
    if let Some(window) = app.get_webview_window(ANYWHERE_LABEL) {
        place_anywhere_bar(&window);
        let _ = window.show();
        return Ok(());
    }
    ...
    let window = WebviewWindowBuilder::new(
        app,
        ANYWHERE_LABEL,
        WebviewUrl::App("anywhere.html".into()),
    )
    .title("Deck Anywhere")
    .inner_size(1280.0, ANYWHERE_HEIGHT_LOGICAL)
    .decorations(false)
    .transparent(false)
    .always_on_top(true)
    .skip_taskbar(true)
...
pub fn close_anywhere_window(app: &AppHandle) -> AppResult<()> {
    if let Some(window) = app.get_webview_window(ANYWHERE_LABEL) {
        window.close()?;
    }
    Ok(())
...
pub fn dock_slideout_position(app: &AppHandle, width: u32, height: u32) -> Position {
    let window = app.get_webview_window(ANYWHERE_LABEL).or_else(|| {
    ...
    // Native top taskbar (Windhawk taskbar-on-top) when the anywhere bar is off.
    let hud = if app.get_webview_window(ANYWHERE_LABEL).is_none() {
        (56.0 * scale).round() as i32
    } else {
        (ANYWHERE_HEIGHT_LOGICAL * scale).round() as i32
    };
...
pub fn path_to_url(path: &PathBuf, port: u16) -> AppResult<WebviewUrl> {
    let url = crate::protocol::widget_http_url(path, port)?;
